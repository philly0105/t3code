...
[`builtInDrivers.ts`][drivers] exports `BUILT_IN_DRIVERS` with seven entries in this fork:
...
The fork also keeps the older [`AgyDriver`][agy] under the separate `agy` driver kind. It uses the
external `agy` CLI and remains available for existing instances, Windows credential profiles, and
on-demand `/usage` quota reads. New instances should use the official `antigravity` driver unless
they need one of those Agy-specific behaviors.
