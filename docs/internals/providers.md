...
This fork also keeps the older `agy` driver kind beside `antigravity`. It drives the external `agy`
CLI and stays available for existing instances, Windows credential profiles, and on-demand `/usage`
quota reads. New instances should use the official `antigravity` driver unless they need one of
those behaviors.
See the [driver](../../apps/server/src/provider/Drivers/AgyDriver.ts).
...
On-demand `/usage` quota reads spawn a short-lived ACP process, authenticate headless, and call the
`x.ai/billing` extension. They never send `session/new`, so they cannot boot MCP servers or spend a
turn; headless `grok -p /usage` is a real model turn and is not used. `grok agent stdio` on current
CLIs answers `x.ai/billing` with method-not-found, so those reads fall back to
`GET {cli-chat-proxy}/billing?format=credits` with the cached `~/.grok/auth.json` token, the same
credits config the extension handler fetches.
