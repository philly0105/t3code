...
| Antigravity | Antigravity CLI                                       | `agy`          | sign in through `agy` |

Codex and Claude are on by default. Cursor, Grok Build, OpenCode, and Antigravity are off by
default; turn them on in **Settings** → the provider's card when you want to use them. Note that
Antigravity threads run without approval prompts, which is a behavior difference you should know
before enabling it.
