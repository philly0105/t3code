...
// Only the interaction-mode commands are recognised when typed and sent
// bare. `/model` and `/usage` open UI instead, so they must be picked from the
// menu and never travel to the provider as a prompt.
